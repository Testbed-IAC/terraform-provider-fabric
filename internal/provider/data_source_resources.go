...
func (d *ResourcesDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"start_date":    schema.StringAttribute{Optional: true, Validators: []validator.String{fabricTimeValidator{}}, Description: "Availability start time in FABRIC or RFC3339 format.", MarkdownDescription: "Availability start time in FABRIC or RFC3339 format."},
			"end_date":      schema.StringAttribute{Optional: true, Validators: []validator.String{fabricTimeValidator{}}, Description: "Availability end time in FABRIC or RFC3339 format.", MarkdownDescription: "Availability end time in FABRIC or RFC3339 format."},
...
func resourcesQuery(config ResourcesDataSourceModel, level int64) (fabricclient.ResourcesQuery, error) {
	startDate, err := canonicalFabricTimeString(stringValue(config.StartDate))
	if err != nil {
		return fabricclient.ResourcesQuery{}, fmt.Errorf("%w: %w", errStartDate, err)
	}
	endDate, err := canonicalFabricTimeString(stringValue(config.EndDate))
	if err != nil {
		return fabricclient.ResourcesQuery{}, fmt.Errorf("%w: %w", errEndDate, err)
	}
	return fabricclient.ResourcesQuery{
		Level:        int32(level),
		ForceRefresh: boolValue(config.ForceRefresh),
		StartDate:    startDate,
		EndDate:      endDate,
		Includes:     stringValue(config.Includes),
		Excludes:     stringValue(config.Excludes),
		GraphFormat:  stringValue(config.GraphFormat),
	}, nil
}
