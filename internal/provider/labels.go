...
var errBGPKeyRequiresASN = errors.New("bgp_key requires asn")
...
func (m labelsModel) toFIM() (*sliver.Labels, error) {
	labels := sliver.Labels{
		BDF:            stringValue(m.BDF),
		MAC:            stringValue(m.MAC),
		IPv4:           stringValue(m.IPv4),
		IPv4Range:      stringValue(m.IPv4Range),
		IPv4Subnet:     stringValue(m.IPv4Subnet),
		IPv6:           stringValue(m.IPv6),
		IPv6Range:      stringValue(m.IPv6Range),
		IPv6Subnet:     stringValue(m.IPv6Subnet),
		VLAN:           stringValue(m.VLAN),
		VLANRange:      stringValue(m.VLANRange),
		InnerVLAN:      stringValue(m.InnerVLAN),
		ASN:            stringValue(m.ASN),
		Instance:       stringValue(m.Instance),
		InstanceParent: stringValue(m.InstanceParent),
		LocalName:      stringValue(m.LocalName),
		LocalType:      stringValue(m.LocalType),
		DeviceName:     stringValue(m.DeviceName),
		BGPKey:         stringValue(m.BGPKey),
		AccountID:      stringValue(m.AccountID),
		Region:         stringValue(m.Region),
		USBID:          stringValue(m.USBID),
	}
	...
	if labels.BGPKey != "" && labels.ASN == "" {
		return nil, fmt.Errorf("validating labels: %w", errBGPKeyRequiresASN)
	}
	return &labels, nil
}
...
func validateLabelConfiguration(model SliceResourceModel, diags *diag.Diagnostics) {
	...
	for i, network := range model.Networks {
		...
		for j, iface := range network.Interfaces {
			validateLabelsBlock(iface.Labels, networkPath.AtName("interface").AtListIndex(j).AtName("labels"), diags)
		}
	}
}
...
func validateLabelsBlock(labels labelsModel, labelsPath path.Path, diags *diag.Diagnostics) {
	...
	if err != nil {
		if errors.Is(err, errBGPKeyRequiresASN) {
...
func validateNodeHost(node NodeModel, nodePath path.Path, diags *diag.Diagnostics) {
	...
	instanceParent := stringValue(node.Labels.InstanceParent)
