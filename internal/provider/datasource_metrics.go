...
// NewMetricsDataSource returns the FABRIC metrics data source.
...
func (d *MetricsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	results, err := d.client.GetMetricsOverview(ctx, fabricclient.MetricsQuery{ExcludedProjects: stringValues(config.ExcludedProjects)})
	if err != nil {
