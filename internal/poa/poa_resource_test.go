...
func poaRaw(t *testing.T, ctx context.Context, model ResourceModel) tfsdk.Plan {
	t.Helper()
...
func poaState(t *testing.T, ctx context.Context, model ResourceModel) tfsdk.State {
	t.Helper()
	...
	cases := []struct {
		...
		},
		{
			...
			model: ResourceModel{
				...
				VCPUCPUMap: []VCPUCPUModel{{VCPU: types.StringValue("0"), CPU: types.StringValue("2")}},
		...
		},
		{
			...
			model: ResourceModel{
				...
				Keys:      []KeyModel{{Key: types.StringValue("ssh-ed25519 AAAA"), Comment: types.StringValue("test")}},
			},
			check: func(t *testing.T, got fabricclient.POARequest) {
				t.Helper()
